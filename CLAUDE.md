...
Current placeholder mechanics: **House** generates 1 labor/tick (cap 10). **Farm** generates 1 sun/tick then bulk-converts sun + labor → food. **Forest** grows trees logistically then bulk-converts tree + labor → wood. **Storage** holds up to 100 of a single storable resource type, no conversion. Time can be paused/resumed with the space key (`togglePause`) or the Time controls in the top bar.
...
`RESOURCE_TYPES` is the catalog of transportable/storable units: `food` 🥕, `sun` ☀️ (`transportable: false`), `labor` 💪 (`storable: false`), `tree` 🌳 (`transportable: false`), `wood` 🪵. It drives the edge resource dropdown and the emoji for inventory badges/travelers. **`getResource(id)`** (Map-backed) is the single lookup — don't write `RESOURCE_TYPES.find(...)` inline.
